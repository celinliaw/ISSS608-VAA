...
exam %>%
  ggplot(aes(x = RACE, y = MATHS)) +
  stat_pointinterval(
    aes(color = factor(.width), linewidth = factor(.width)),
    .width = c(0.95, 0.99),
    method = "qi",
    point_interval = median_qi,
    position = position_dodge(width = 0.5)
  ) +
  scale_color_manual(values = c("0.95" = "black", "0.99" = "gray50")) +
  scale_linewidth_manual(values = c("0.95" = 1.5, "0.99" = 0.8)) +
  labs(
    title = "Visualising Confidence Intervals of Median Math Score",
    subtitle = "Median point with 95% and 99% credible intervals",
    color = "Interval Width",
    linewidth = "Interval Width"
  ) +
  theme_minimal()
