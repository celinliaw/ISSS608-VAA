...
-   Analysis is split into the different population age groups which makes it easier to make analysis based on the age groups
...
-   The graphs are ordered in descending order according to their population (from the largest population to the 10th largest population), making it easier for viewing based on the rank

**3 Areas for Further Improvements:**

-   The colours for the bar charts are in shades of grey which makes it hard to differentiate the different subzones. It would be better if specific colours are used for each area

-   There is only 1 tab for comparison between 2 groups (young and youth dependents). It would be better if more side-by side comparisons can be made

-   The chart also only shows the top 10 subzones but the other subzones were not shown. It would be better if the lowest 10 subzones or all subzones can be shown.
...
The code <code style="color: blue;">#\| code-fold: true</code> is also added at the top of the codes to make sure the codes are collapsible and the other graphs can be seen easily side by side. They have also been displayed from the youngest to the oldest.
