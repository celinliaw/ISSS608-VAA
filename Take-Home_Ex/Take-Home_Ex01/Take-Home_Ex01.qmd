...
-   Is the current number of working adults enough to support the number of retirees in Singapore
...
The data that has been used has been mentioned in Chapter 1.2 above. his dataset will be imported as *resident_data.*
...
There are no missing values in any of the rows and the next step will be filtering and renaming the data.
...
A new object called **resident_data_clean** is created where it will renamed **PA, SZ** and **Pop** for easy reading by using the <code style="color: blue;">rename()</code> function.
...
For the Age column, it is slightly special as <code style="color: blue;">levels = as.character</code> function in R sorts alphabetically by default and "10" will come before "2" instead. But by using this function, R will be forced to treat the ages in true numeric order when plotting or tabulating them. A new column, **Age_numeric**, is also created in the new object.
...
As there are too many planning areas, to make things easier to visualise, the planning areas have been further categorised into 5 areas: North, North-East, East, West and Central. This is done by using the categories on the [Urban Redevelopment Authority(URA)](https://eservice.ura.gov.sg/maps/?service=MP) website:
...
Now that data wrangling is complete, 3 key visualisations will be performed on this data set.

# 3. Data Visualisation

## 3.1 Pie Chart By Regions

First, groups are being created using <code style="color: blue;">groupby()</code> to group them by regions and sex (Eg. East Males and East Females) so that the total population of each group can be found using <code style="color: blue;">sum()</code>. After that, <code style="color: blue;">.groups = "drop"</code> will remove the groups as ggplot doesn't use groupings to plot and some functions that "mutate" may not turn out as expected.

Next, <code style="color: blue;">ordered_levels</code> is created to define the stacking order for the pie slices in an anti-clockwise direction.

<code style="color: blue;">mutate()</code> is used to add new columns to the pie_all_regions object.
...
<code style="color: blue;">geom_text</code> is used to add percentage labels inside each pie slice with **size of 3** and **text colour black.**
...
<code style="color: blue;">scale_fill_manual</code> is used to set custom colours for each Region-Sex.
...
Additionally, the population in the **Age range of 0 to 29** has consistently **more males that females** in all areas **except in the Central Area.** However, those in the **Age range 30 and above** has **more females than males** in all areas with those aged 60 to 69 in the North area being the exception.

## 3.3 Bar Graph of Dependency Ratio

The **dependency ratio** is a demographic measure that indicates the **ratio of dependents** (those typically not in the labour force) t**o the working-age population**. **Dependents** are usually defined as those u**nder 15 and above 65 years old**.
...
<code style="color: blue;">mutate()</code> is used to change the item to numeric so that i am able to use \<= and \>= to create the new custom **Age_Group of 24 to 67** and **68 and above.**
...
ggplot(adjusted_ageing_ratio_by_region, aes(x = reorder(Region, Adjusted_Ageing_Ratio), y = Adjusted_Ageing_Ratio)) +
  ...
  labs(
    title = "Ratio of Dependents(<=14YO & >=65YO) to Working Adults(15-65YO)",
    x = "Region",
    y = "Dependency Ratio"
  ) +
  coord_flip() +
...
In this exercise, data cleaning and visualisation of data was performed to examine the relationship between Age, Gender, Region and Population in Singapore. The key findings are:
...
-   There is consistently **more population in the Age Range 30-69** as compared to **Age Range 0 to 29**, showing a decrease in birth rate in Singapore
...
With relation to the codes used for analysis, **ggplot2** is a powerful package for exploratory data analysis when paired with **dplyr** which allows mutation of the data to usable formats. The original dataset consists of many areas which makes it difficult to find a direction and thus was further categorised into subgroups using the dplyr functions. Additional functions like **geom_col, geom_text, coord_flip, coord_cartesian and coor_polar** were further utilised to create the visualisations for visual analysis.
